...
u.experiment <- read_csv("uExperiment.csv", col_types = cols())
```

Load h for each chamber set point and posture from h.Rmd
```{r echo=TRUE, message=FALSE, warning=FALSE}
mean.h <- read_csv("h.csv", col_types = cols()) %>%
...
baseline <- experiment %>%
  ...
  summarise(Baseline.Teq = mean(Teq), Baseline.u.Teq = sqrt(sum(u.Teq^2)))
```
...
thermal.effect <- experiment %>%
  ...
  select(1:10, Alias, Tambient, u.Tambient, Teq, u.Teq) %>%
  left_join(baseline) %>%
...
thermal.effect_group <- thermal.effect %>%
  ...
  group_by(Alias) %>%
  summarise(ThermalEffect = mean(ThermalEffect), u.ThermalEffect = sqrt(sum(u.ThermalEffect^2))) %>%
  ungroup() %>%
  mutate(Alias2 = Alias) %>%
...
```{r}
thermal.effect_labels <- read_csv("thermal.effect_labels.csv")
...
Load PCS power and uncertainty
```{r}
pcs.power <- read_csv("pcs.power.csv", col_types = cols()) %>%
  rename(Power = Mean, u.Power = u)
```
Combine thermal effect and power
```{r}
pcs <- thermal.effect %>%
  filter(PCS != "None") %>%
  mutate(Baseline = ifelse(Clothing == "Light", "Baseline", "All Passive")) %>%
  left_join(pcs.power) %>%
  mutate(ThermalEfficiency = ThermalEffect/Power) %>%
  mutate(u.ThermalEfficiency = abs(ThermalEffect/Power)*sqrt((u.ThermalEffect/ThermalEffect)^2)+(u.Power/Power)^2) %>%
  filter(!is.na(ThermalEfficiency)) %>%
  mutate(System = ifelse(Clothing == "Light", "Active only", "Active + Passive")) %>%
  rename(Label = PCS) %>%
  group_by(Chamber.SetPoint, Label, System) %>%
  summarise(ThermalEfficiency = mean(ThermalEfficiency), u.ThermalEfficiency = sqrt(sum(u.ThermalEfficiency^2))) %>%
  mutate(Abs.ThermalEfficiency = abs(ThermalEfficiency)) %>%
  mutate(System = factor(System, levels = c("Passive only", "Active only", "Active + Passive"))) %>%
  mutate(Mode = ifelse(Chamber.SetPoint == 16, "Heating", "Cooling")) %>%
  mutate(Mode = factor(Mode, levels = c("Heating", "Cooling"))) %>%
   mutate(Label = factor(Label, levels = c("HydropoweredMP", "CeilingFanLow", "CeilingFanHigh", "PedestalFanHigh", "PedestalFanLow", "HeatedBlanket", "ElectricMPHigh", "ElectricMPLow", "Posture only","Clothing only", "Bed only", "Emergency blanket only", "Bedding only", "All passive")))


```

```{r}
ggplot(pcs, aes(x = Label, y = Abs.ThermalEfficiency)) +
  #geom_hline(yintercept = 0, linetype = "dashed", colour = "gray25") +
  #geom_hline(data = all_passive, aes(yintercept = Abs.ThermalEffect), linetype = "dotted", colour = "gray25") +
  geom_errorbar(aes(ymin = Abs.ThermalEfficiency - (2*u.ThermalEfficiency), ymax = Abs.ThermalEfficiency + (2*u.ThermalEfficiency)), width = 0.5) +
  geom_point(aes(colour = System, shape = System), size = 3) +
  #scale_y_continuous(breaks = seq(0, 8, 2), limits = c(-1, 8)) +
  #scale_colour_manual(values = c("#E69F00", "#009E73")) +
  scale_shape_manual(values = c(15, 17)) +
  theme_custom() +
  theme(legend.title = element_blank()) +
  xlab("") +
  ylab("Absolute value of heating or cooling efficiency (deg C/W)") +
  coord_flip() +
  facet_grid(vars(Mode), scales = "free") 
```
