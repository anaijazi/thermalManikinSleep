...
data_files <- list.files(path = data_dir, pattern = "_KAW.csv", full.names = TRUE)
...
HeatedBlanket_Hobo <- read_csv("data/Power/HeatedBlanket_HOBO.csv", skip = 2, col_names = c("Number", "Time", "Current"), col_types = cols()) %>%
  select(-Number) %>%
  mutate(Time = mdy_hms(Time)) %>%
  mutate(Power = Current*120) %>%
  mutate(Baseline = case_when(between(Time, mdy_hms("3/4/2022  2:14:00 PM"), mdy_hms("3/4/2022  3:56:00 PM")) ~ "Baseline",
                              between(Time, mdy_hms("3/4/2022  4:07:00 PM"), mdy_hms("3/4/2022  5:51:00 PM")) ~ "All Passive")) %>%
  drop_na(Baseline) %>%
  select(-Current) %>%
  group_by(Baseline)
```
...
\
```{r}
HeatedBlanket_KAW <- PCS_data %>%
  filter(PCS == "HeatedBlanket") %>%
  mutate(Meter = "KAW") %>%
  select(Time, Power, Baseline, Meter)
...
HeatedBlanket_data <- full_join(HeatedBlanket_KAW, HeatedBlanket_Hobo)
HeatedBlanket_data
```

```{r}
ggplot(HeatedBlanket_Hobo, aes(x = Time, y = Power)) +
  geom_line(aes(linetype = Baseline)) +
  theme_custom()
```
```{r}
HeatedBlanket_Hobo_avg <- HeatedBlanket_Hobo %>%
  group_by(Baseline) %>%
